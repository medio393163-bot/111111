          ...
          <div className="grid grid-cols-1 xl:grid-cols-12 gap-4">
            ...
            <div className="xl:col-span-8 space-y-1.5">
              <div className="flex justify-between items-center text-[10px] text-slate-450 font-semibold">
                ...
                <span className="font-mono text-slate-400">Format: Standard Plain-text Compliance</span>
              </div>

              <textarea
                value={activePolicyTab === 'refund' ? refundPolicy : activePolicyTab === 'privacy' ? privacyPolicy : tosPolicy}
                onChange={e => {
                  if (activePolicyTab === 'refund') setRefundPolicy(e.target.value);
                  if (activePolicyTab === 'privacy') setPrivacyPolicy(e.target.value);
                  if (activePolicyTab === 'tos') setTosPolicy(e.target.value);
                }}
                rows={10}
                placeholder="在此框输入具体的法律政策。推荐直接点击左侧「导入标准欧盟条款模版」一键补齐内容。"
                className="w-full text-[11px] font-mono leading-relaxed bg-slate-50 border border-slate-200/80 p-3 rounded-xl focus:bg-white focus:outline-none focus:ring-1 focus:ring-indigo-600 focus:border-indigo-600 font-medium text-slate-800"
              />
            </div>
